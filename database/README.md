# 数据库测试数据说明

本目录包含TodoList项目的数据库表结构和测试数据脚本，方便快速搭建测试环境。

## 📁 文件说明

```
database/
├── schema.sql                 # 数据库表结构定义（包含所有表和 PDF 字段）
├── seed.sql                   # 测试数据（需要先更新密码哈希）
├── generate-passwords.js      # 密码哈希生成工具
├── setup-all.sh              # 一键安装脚本 (Linux/Mac) ⭐ 推荐
├── setup-all.bat             # 一键安装脚本 (Windows) ⭐ 推荐
├── import.sh                 # 分步导入脚本 (Linux/Mac)
├── import.bat                # 分步导入脚本 (Windows)
├── railway-import.sh         # Railway平台导入脚本 (Linux/Mac) ⭐ Railway部署
├── railway-import.bat        # Railway平台导入脚本 (Windows) ⭐ Railway部署
├── add-report-pdf-fields.sql # PDF 字段迁移脚本（仅用于旧数据库升级）
├── RAILWAY_QUICK_IMPORT.md   # Railway快速导入指南 ⭐ Railway部署
├── RAILWAY_IMPORT_GUIDE.md   # Railway完整导入指南
└── README.md                 # 本文档
```

## 🚀 快速开始（一键安装）⭐

### 方法 1：使用一键安装脚本（推荐）

**Windows:**
```cmd
cd database
setup-all.bat
```

**Linux/Mac:**
```bash
cd database
chmod +x setup-all.sh
./setup-all.sh
```

这将自动完成：
- ✅ 创建所有数据库表（users, todos, orders, reports）
- ✅ 添加 PDF 存储字段（pdfUrl, pdfKey）
- ✅ 导入测试数据（5个测试用户，290条待办，5个订单）
- ✅ 显示测试账号信息

⚠️ **警告**：脚本会删除并重建所有表，现有数据将丢失！

---

## ☁️ Railway 平台部署 ⭐

如果你要将数据库部署到 Railway 平台，请使用以下方法：

### 方法 1：使用 Railway 专用导入脚本（推荐）

**Windows:**
```cmd
cd database
# 1. 编辑 railway-import.bat，填入Railway数据库连接信息
# 2. 运行脚本
railway-import.bat
```

**Linux/Mac:**
```bash
cd database
# 1. 编辑 railway-import.sh，填入Railway数据库连接信息
chmod +x railway-import.sh
# 2. 运行脚本
./railway-import.sh
```

### 方法 2：使用 Railway CLI（最简单）

```bash
# 1. 安装并登录 Railway CLI
railway login
railway link

# 2. 导入数据
cd database
railway run psql -f schema.sql
railway run psql -f seed.sql
```

### 📖 详细文档

- 🚀 **快速入门**: [RAILWAY_QUICK_IMPORT.md](./RAILWAY_QUICK_IMPORT.md) - 3种快速导入方法
- 📚 **完整指南**: [RAILWAY_IMPORT_GUIDE.md](./RAILWAY_IMPORT_GUIDE.md) - 详细步骤和问题排查

---

## 🔧 分步安装（旧方法）

### 1. 生成密码哈希（首次使用）

测试数据中的用户密码需要使用bcrypt加密。首先生成密码哈希：

```bash
# 确保已安装依赖
cd backend
npm install

# 运行密码生成脚本
node ../database/generate-passwords.js
```

将生成的密码哈希复制到 `database/seed.sql` 文件中，替换对应的占位符：
- `$2b$10$YourHashedPasswordHere1`
- `$2b$10$YourHashedPasswordHere2`
- `$2b$10$YourHashedPasswordHere3`
- `$2b$10$YourHashedPasswordHere4`

### 2. 配置数据库连接

#### 方式一：使用环境变量

```bash
# Linux/Mac
export DB_HOST=localhost
export DB_PORT=5432
export DB_NAME=todolist
export DB_USER=postgres
export DB_PASSWORD=your_password

# Windows PowerShell
$env:DB_HOST="localhost"
$env:DB_PORT="5432"
$env:DB_NAME="todolist"
$env:DB_USER="postgres"
$env:DB_PASSWORD="your_password"
```

#### 方式二：使用 .env 文件

脚本会自动读取 `backend/.env` 文件中的配置。

### 3. 导入数据

#### Linux/Mac

```bash
# 赋予执行权限
chmod +x database/import.sh

# 运行导入脚本
./database/import.sh
```

#### Windows

```cmd
# 双击运行
database\import.bat

# 或在命令行中运行
database\import.bat
```

#### 手动导入（使用psql）

```bash
# 创建表结构
psql -h localhost -p 5432 -U postgres -d todolist -f database/schema.sql

# 导入测试数据
psql -h localhost -p 5432 -U postgres -d todolist -f database/seed.sql
```

#### 使用Docker中的PostgreSQL

```bash
# 如果使用docker-compose启动的PostgreSQL
docker-compose exec postgres psql -U postgres -d todolist -f /path/to/schema.sql
docker-compose exec postgres psql -U postgres -d todolist -f /path/to/seed.sql

# 或从宿主机导入
docker exec -i $(docker-compose ps -q postgres) psql -U postgres -d todolist < database/schema.sql
docker exec -i $(docker-compose ps -q postgres) psql -U postgres -d todolist < database/seed.sql
```

## 👥 测试账号

导入成功后，可以使用以下测试账号登录：

### 1. 免费用户
- **邮箱**: `free@test.com`
- **密码**: `test123456`
- **特点**: 免费账户，待办事项较少
- **用途**: 测试基础功能和Pro升级流程

### 2. Pro用户（会员有效期1年）
- **手机**: `13800138000`
- **密码**: `test123456`
- **特点**: 活跃Pro会员，待办事项丰富
- **用途**: 测试Pro功能（AI分析等）

### 3. Pro用户（会员即将到期）
- **邮箱**: `pro@test.com`
- **密码**: `test123456`
- **特点**: 会员5天后到期，有到期提醒任务
- **用途**: 测试会员到期提醒和续费流程

### 4. 过期Pro用户
- **邮箱**: `expired@test.com`
- **密码**: `test123456`
- **特点**: Pro会员已过期
- **用途**: 测试会员过期后的功能限制

### 5. 微信用户
- **微信OpenID**: `wx_test_openid_12345`
- **昵称**: `微信用户张三`
- **特点**: 通过微信登录的用户
- **用途**: 测试微信登录功能（需模拟）

## 📊 测试数据统计

导入完成后的数据概览：

- **用户数**: 5
  - Pro用户: 3
  - 免费用户: 2
  
- **待办事项数**: 290条
  - 免费用户小明: 30条
  - Pro会员小红: 120条
  - Pro会员小李: 80条
  - 过期会员小王: 40条
  - 微信用户张三: 20条
  
- **订单数**: 5
  - 已支付: 3条
  - 待支付: 1条
  - 已取消: 1条

### 待办事项覆盖场景

1. **重要性级别**: A、B、C、D 全覆盖
2. **紧急程度**: 1-5 全覆盖
3. **状态**: pending、in_progress、completed、cancelled 全覆盖
4. **截止日期**: 过期、即将到期、未来、无截止日期
5. **典型场景**: 
   - 紧急Bug修复
   - 项目演示准备
   - 学习计划
   - 代码重构
   - 日常琐事

## 🔧 数据库表结构

### users 表
用户信息表，包含：
- 基本信息：手机、邮箱、昵称、头像
- 登录方式：phone、email、wechat
- 会员信息：账户类型、订阅状态、到期时间

### todos 表
待办事项表，包含：
- 内容：标题、描述
- 优先级：重要性（A-D）、紧急程度（1-5）
- 状态：pending、in_progress、completed、cancelled
- 时间：截止日期、完成时间

### orders 表
订单表，包含：
- 订单信息：订单号、金额、套餐类型
- 支付信息：支付方式、交易号、支付时间
- 状态：pending、paid、failed、refunded、cancelled

## 🔄 重置数据库

如果需要重置数据库到初始状态，可以：

### 方式一：重新运行一键安装脚本（推荐）

```bash
cd database
./setup-all.sh  # Linux/Mac
setup-all.bat   # Windows
```

### 方式二：运行分步导入脚本

```bash
cd database
./import.sh  # Linux/Mac
import.bat   # Windows
```

### 方式二：手动重置

```bash
# 删除所有数据（保留表结构）
psql -U postgres -d todolist -c "TRUNCATE TABLE orders, todos, users CASCADE;"

# 重新导入测试数据
psql -U postgres -d todolist -f database/seed.sql
```

### 方式三：完全重建

```bash
# 删除所有表和数据
psql -U postgres -d todolist -f database/schema.sql

# 导入测试数据
psql -U postgres -d todolist -f database/seed.sql
```

## 💡 使用建议

1. **开发环境**：使用测试数据快速验证功能
2. **演示环境**：展示完整的功能和数据场景
3. **测试环境**：每次测试前重置到已知状态
4. **生产环境**：⚠️ **请勿**导入测试数据

## 🐛 常见问题

### Q1: 导入时提示 "psql: command not found"

**解决方案**：
- 安装PostgreSQL客户端工具
- 将PostgreSQL的bin目录添加到PATH环境变量

### Q2: 密码验证失败

**解决方案**：
- 检查环境变量 `DB_PASSWORD` 是否正确
- 或在运行psql时手动输入密码

### Q3: 用户无法登录

**解决方案**：
- 确保已运行 `generate-passwords.js` 生成密码哈希
- 确保已将生成的哈希值更新到 `seed.sql` 文件中
- 重新导入数据

### Q4: 数据库连接失败

**解决方案**：
- 检查PostgreSQL服务是否启动
- 检查数据库名称、端口是否正确
- 检查防火墙设置

### Q5: 中文显示乱码

**解决方案**：
```bash
# 设置客户端编码为UTF-8
psql -U postgres -d todolist -c "SET CLIENT_ENCODING TO 'UTF8';"
```

## 📝 自定义测试数据

如果需要添加自定义测试数据：

1. **编辑 `seed.sql`**：添加新的INSERT语句
2. **保持一致性**：确保外键关系正确（userId等）
3. **注意时间**：使用相对时间（NOW() + INTERVAL）保持数据新鲜度
4. **重新导入**：运行导入脚本应用更改

## 🔐 安全提示

1. ⚠️ 测试密码 `test123456` 仅用于开发/测试环境
2. ⚠️ 生产环境必须使用强密码
3. ⚠️ 不要将 `.env` 文件提交到版本控制
4. ⚠️ 定期更换生产环境的数据库密码

## 📚 相关文档

- [NestJS TypeORM](https://docs.nestjs.com/techniques/database)
- [PostgreSQL Documentation](https://www.postgresql.org/docs/)
- [bcrypt](https://github.com/kelektiv/node.bcrypt.js)

---

最后更新：2026-01-04

